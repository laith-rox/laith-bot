...
def paper_fill(decision, bar):
    if decision.get('decision_time') is not None and bar.start.timestamp() <= decision['decision_time']: return None
    direction = 1 if decision['side'] == 'BUY' else -1
    if not (direction*(bar.open-decision['sl']) > 0 and direction*(decision['tp1']-bar.open) > 0): return None
    if abs(bar.open-decision['price']) > .5*decision['atr']: return None
    risk = abs(bar.open-decision['sl']); reward = direction*(decision['tp2']-bar.open)
    if risk > decision.get('max_fill_risk', float('inf')) or reward/risk < decision.get('min_reward_risk', 0): return None
    filled = dict(decision, price=bar.open); trade = make_trade(filled, bar.start)
    trade.update(status='active', announced=bar.start.timestamp(), rule=decision.get('rule','fast-v1'), setup=decision.get('reason'), context=decision.get('context'))
    for key in ('decision_time','source_age_at_decision_seconds','execution_model'):
        if key in decision: trade[key] = decision[key]
    return trade
...
def advance_fast(original, bars):
    ...
    for bar in bars:
        ...
        if bar.end.timestamp() <= (trade['last_end'] or trade['announced']): continue
        if bar.start.timestamp() >= trade['announced'] + MAX_HOLD:
            gap = bar.start.timestamp() > (trade['last_end'] or trade['announced']); risk = abs(trade['entry']-trade['initial_sl'])
            r = (1 if trade['side']=='BUY' else -1)*(bar.open-trade['entry'])/risk
            trade.update(status='closed', outcome='TIME_LIMIT', exit=bar.open, closed=bar.start.timestamp(), data_gap=trade['data_gap'] or gap, r=None if trade['data_gap'] or gap or trade['delivery_uncertain'] else r)
            break
        if trade['last_end'] is None and bar.start.timestamp() > trade['announced']: trade['data_gap'] = True
...
class MinuteMarket:
    ...
    def _fetch(self, now, end=None, size=600, fresh=True):
        ...
        if end is not None and fast_window(now):
            raise DataError('minute_research_deferred_during_fast_window')
        params = {'symbol':'XAU/USD','interval':'1min','outputsize':size,'timezone':'UTC','order':'ASC','format':'JSON','apikey':self.key}
        ...
        try:
            ...
            if response.status_code == 429: raise DataError('minute_quota_reached')
            if response.status_code != 200: raise DataError('minute_http_' + str(response.status_code))
            payload = response.json()
        except (requests.RequestException, ValueError): raise DataError('minute_connection_or_json_failed') from None
        if isinstance(payload, dict) and payload.get('status') == 'error':
            code = payload.get('code'); raise DataError('minute_quota_reached' if code == 429 else 'minute_access_rejected')
